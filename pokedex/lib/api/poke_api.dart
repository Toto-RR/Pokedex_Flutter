...
class ApiService {
  ...
  static Future<Map<String, dynamic>> fetchPokemonInfo(String name) async {
  ...
  static Future<Map<String, dynamic>> fetchGenerationInfo(String generationName) async {
    final response = await http.get(Uri.parse('https://pokeapi.co/api/v2/generation/$generationName/'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
      throw Exception('Failed to load Generation Info');
    }
  }

  static Future<Map<String, dynamic>> fetchTypeInfo(String typeName) async {
  ...
  static Map<String, Color> typeColors = {
    ...
    'psychic': const Color.fromARGB(255, 218, 100, 131),
    ...
    'fighting': const Color.fromARGB(255, 199, 44, 22),
